...
function CreateShowroomVehicle(showroom, vehicleName)
  ...
  while HasModelLoaded(vehicleName) do
    SetModelAsNoLongerNeeded(vehicleName)

    if showroom == 'pdm' then
      CalculateCamPos(vehicleName, 0)
      return CreateVehicle(vehicleName, 580.4382, -191.2608, 49.9501, 39.0664, false, false)
...
function RenderStaticCam(showroom)
  ...
  if (showroom == 'pdm') then -- if (showroom == 'fastlane') then
    SetCamCoord(staticCam, 575.6862, -191.1813, 50.6325)
    SetCamRot(staticCam, 0.0, 0.0, 270.7538)
    SetFocusPosAndVel(575.6862, -191.1813, 50.6325, 0.0, 0.0, 0.0)
...
exports['qb-target']:AddBoxZone("showroom", vector3(561.0930, -245.3843, 49.9806), 5.0, 6.6, {
  minZ = 49.9806 - 2.0,
  maxZ = 49.9806 + 2.0,
  name = "showroom",
  heading = 246.4827,
