...
local function lockpickFinish(success)
    ...
    if success then
        TriggerServerEvent('hud:server:GainStress', math.random(1, 6))
        QBCore.Functions.Notify('Opened Door!', 'success')
        SetVehicleDoorsLocked(vehicle, 1)
    else
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 12))
    ...
    while true do 
		if DoesEntityExist(GetVehiclePedIsTryingToEnter(PlayerPedId())) then
            ...
            elseif (IsVehicleDoorFullyOpen(veh, 0)) or (IsVehicleDoorFullyOpen(veh, 1)) or (IsVehicleDoorFullyOpen(veh, 2)) or (IsVehicleDoorFullyOpen(veh, 3)) or not DoesVehicleHaveDoor(veh, 0) then
                SetVehicleDoorsLocked(veh, 1)
            elseif lock == 0 or lock == 7 or lock == 3 or lock == 2 then
                SetVehicleDoorsLocked(veh, 2)
            end
        end
        Wait(500)
