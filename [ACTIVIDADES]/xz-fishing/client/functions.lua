function TryToFish()
    ...
    end, Config.FishingItems["fishingrod"]["name"], Config.FishingItems["bait"]["name"])
end
...
CastBait = function(rodHandle, castLocation)
    ...
    QBCore.Functions.Notify("Waiting for a fish to bite...", "primary", "10000")
    TriggerServerEvent('qb-fishing:removeItemV2', "bait")
...
function IsInWater()
    ...
    QBCore.Functions.Notify("Checking fishing location...", "primary", "3000")
...
function HasFishingBait()
    local rtval = false
    rtval = QBCore.Functions.HasItem("bait")
