window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		$('#profile-money').empty();
		$('#profile-money').append(new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.money));
		$('#bank-money').empty();
		$('#bank-money').append(new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.money));
		
		$('#profile-money-earned').empty();
		$('#profile-money-earned').append(new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(users.total_earned));
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								...
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Reward: ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(contract.reward) + `</span></div>
							</div>
						</div>
					</div>
					...
					<div class="card-body pt-0 px-0">
						...
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							...
							<div class="d-flex flex-column">
								<h5 class="mb-0">` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(truck.price) + `</h5>
							</div>
						</div>
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><span>Fix Body (` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format((100-truck.body)*config.valor_reparo.body) + `)</span>
						</div>
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><span>Fix Engine (` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format((100-truck.engine)*config.valor_reparo.engine) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><span>Fix Transmission (` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format((100-truck.transmission)*config.valor_reparo.transmission) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><span>Fix Wheels (` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format((100-truck.wheels)*config.valor_reparo.wheels) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">Price: ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(driver.price_per_km) + `/km</p>
							<hr>
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">Price: ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(driver.price_per_km) + `/km</span>
									</div>
									<div class="ml-3">
		...
		$('#loan-title').empty();
		$('#loan-title').append('Get loans to invest in your company (Maximum active loans: ' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.max_emprestimo) + ')');
		
		$('#loan-1').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[0][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[0][1]) + ' at day</span>';
		$('#loan-1').append(list_item);
		$('#loan-2').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[1][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[1][1]) + ' at day</span>';
		$('#loan-2').append(list_item);
		$('#loan-3').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[2][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[2][1]) + ' at day</span>';
		$('#loan-3').append(list_item);
		$('#loan-4').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[3][0]) + '</h3><span>pay ' + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(config.emprestimos[3][1]) + ' at day</span>';
		...
		for (const loan of loans) {
			list_item = `
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							<h6 class="mb-0">` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(loan.loan) + `</h6>
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">Remaining: ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">Daily cost: ` + new Intl.NumberFormat(config.formato.location, { style: 'currency', currency: config.formato.moeda }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
