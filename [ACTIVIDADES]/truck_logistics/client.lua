...
Citizen.CreateThread(function()
	...
	RequestModel(0xC5FEFADE)
	while not HasModelLoaded(0xC5FEFADE) do
		Wait(1000)
	end
	...
	ped = CreatePed(4, 0xC5FEFADE, 1240.19,-3179.84,7.13-1, 0.54, false, true)
	SetEntityHeading(ped, 0.69)
	FreezeEntityPosition(ped, true)
	SetEntityInvincible(ped, true)
	SetBlockingOfNonTemporaryEvents(ped, true)
	TaskPlayAnim(ped,"amb@code_human_wander_clipboard@male@base", "static", 8.0, 0.0, -1, 1, 0, 0, 0, 0)
end)
isLoggedIn = false
PlayerJob = {}
...
AddEventHandler("QBCore:Client:OnPlayerLoaded", function()
	...
	if isLoggedIn then
		addBlip(1239.9354, -3179.2830, 7.1049,478,4,"Camioneros",0.6)
	end
end)
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if not  data.contract_type then
		...
		local camion = GetEntityModel(truck)
		
        local vehicleplate = QBCore.Functions.GetPlate(truck)
		
        TriggerServerEvent('MojiaVehicleKeys:server:AddVehicleKey', vehicleplate, camion)
		rentTruck = true
	end
	...
	while IsEntityAVehicle(truck) and not data.contract_type do 
		timer = 2000
		local ped = PlayerPedId()
		veh = GetVehiclePedIsIn(ped,false)
		for k,v in pairs(Config.empresas) do
