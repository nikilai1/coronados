...
Citizen.CreateThread(function()
	...
	while true do
		...
		local truck = Config.contratos.camiones[math.random(1, #Config.contratos.camiones)]
...
AddEventHandler("truck_logistics:startContract",function(data)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			local query = MySQL.Sync.fetchAll('SELECT * FROM `trucker_available_contracts` WHERE contract_id = ?',{id})
				if query and query[1] then
					query_users = MySQL.Sync.fetchAll('SELECT * FROM `trucker_users` WHERE user_id = ?',{user_id})
						if query_users and query_users[1] then
							if query_users[1].product_type >= query[1].cargo_type then
								if query_users[1].fragile >= query[1].fragile then
									if query_users[1].valuable >= query[1].valuable then
										if query_users[1].fast >= query[1].fast then
											if Config.habilidade_distancia[query_users[1].distance] >= distance then
												if not query[1].contract_type then
													TriggerClientEvent("truck_logistics:startContract",source,query[1],distance,reward,{})
													
												else
													local query_truck = MySQL.Sync.fetchAll("SELECT * FROM `trucker_trucks` WHERE driver = 0 AND user_id = ?", {user_id})
...
AddEventHandler("truck_logistics:spawnTruck",function(truck_id)
	...
	if debug_cooldown[source] == nil then
		...
		local user_id = xPlayer.PlayerData.citizenid
		if user_id then
			local result = MySQL.Sync.fetchAll("SELECT * FROM `trucker_trucks` WHERE truck_id = ?",{tonumber(truck_id)})
...
AddEventHandler("truck_logistics:upgradeSkill",function(data)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		local sql = MySQL.Sync.fetchAll("SELECT * FROM `trucker_users` WHERE user_id = ?", {user_id})
		local query = sql[1];
		if query.skill_points >= (data.value - query[data.id]) then
			MySQL.update.await("UPDATE `trucker_users` SET "..data.id.." = ?, skill_points = ? WHERE user_id = ?", {data.value, (query.skill_points - (data.value - query[data.id])),user_id});
...
AddEventHandler("truck_logistics:repairTruck",function(item)
	...
	local user_id = xPlayer.PlayerData.citizenid
	if user_id then
		local sql = MySQL.Sync.fetchAll("SELECT * FROM `trucker_trucks` WHERE user_id = ? AND driver = 0", {user_id})
		local query = sql[1];
		if query then
			local amount = math.floor((100-(tonumber(query[item])/10)) * Config.valor_reparo[item])
			if amount > 0 then
				if tryGetTruckerMoney(user_id,amount) then
					MySQL.update.await("UPDATE `trucker_trucks` SET "..item.." = 1000 WHERE user_id = ? AND driver = 0", {user_id});
...
function openUI(source, reset)
	...
	if xPlayer then
		...
		if user_id then
			...
			query.config.formato = deepcopy(Config.formato)
