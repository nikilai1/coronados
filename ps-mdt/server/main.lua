...
local MugShotWebhook = 'https://discord.com/api/webhooks/1220137270087127130/VUp4zHn4V9rmqk8wJZ8QUP0vk-yV3sNLUF-6FZq70W05-7pjFBb0OkJw_8ohaYmI3qj9'

-- Clock-in notifications for duty. Add a Discord webhook.
-- Command /mdtleaderboard, will display top players per clock-in hours.
local ClockinWebhook = 'https://discord.com/api/webhooks/1220135957966360690/oWcs5nf05TM5Qgfhqd6a2dc9gmhxq5WFOS_KJ3Akn1dhv8LcJ3obCwSlFDUpVvUvWHVj'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1220137149383442534/imCpLG4a_-guK_BgDc5PoebklkcQwcmCw_5KEf6_7lyNOpqDekECeXBO46_1aNeBTUhT'
--------------------------------
...
QBCore.Commands.Add("pdleaderboard", "Show Police leaderboard", {}, false, function(source, args)
	...
	local result = MySQL.Sync.fetchAll('SELECT firstname, lastname, total_time FROM mdt_clocking WHERE job = @job  ORDER BY total_time DESC', {
		['@job'] = 'police'
	})

    local leaderboard_message = '**Police Leaderboard**\n\n'
    ...
    sendToDiscord(16753920, "Police Leaderboard", leaderboard_message, "CiS | Made by Viejo")
    TriggerClientEvent('QBCore:Notify', source, "Police leaderboard sent to Discord!", 'success')
end)

QBCore.Commands.Add("emsleaderboard", "Show Police leaderboard", {}, false, function(source, args)
    local PlayerData = GetPlayerData(source)
    local job = PlayerData.job.name
    ...
    local leaderboard_message = '**EMS Leaderboard**\n\n'
    ...
    sendToDiscord(16753920, "EMS Leaderboard", leaderboard_message, "CiS | Made by Viejo")
    TriggerClientEvent('QBCore:Notify', source, "EMS leaderboard sent to Discord!", 'success')
end)
RegisterNetEvent("ps-mdt:server:ClockSystem", function()
    ...
    if PlayerData.job.onduty then
		...
		MySQL.Async.insert('INSERT INTO mdt_clocking (user_id, firstname, lastname, clock_in_time) VALUES (:user_id, :firstname, :lastname, :clock_in_time) ON DUPLICATE KEY UPDATE user_id = :user_id, firstname = :firstname, lastname = :lastname, clock_in_time = :clock_in_time', {
			user_id = PlayerData.citizenid .. PlayerData.job.name,
			firstname = firstName,
			lastname = lastName,
			clock_in_time = time
		}, function()
		end)
		sendToDiscord(65280, "MDT Clock-In", 'Player: **' ..  firstName .. " ".. lastName .. '**\n\nJob: **' .. PlayerData.job.name .. '**\n\nRank: **' .. PlayerData.job.grade.name .. '**\n\nStatus: **On Duty**', "CiS | Made by Viejo")
    else
		...
		sendToDiscord(16711680, "MDT Clock-Out", 'Player: **' ..  firstName .. " ".. lastName .. '**\n\nJob: **' .. PlayerData.job.name .. '**\n\nRank: **' .. PlayerData.job.grade.name .. '**\n\nStatus: **Off Duty**\n Total time:' .. time_formatted, "CiS | Made by Viejo")
    end
end)


RegisterNetEvent("ps-mdt:server:ClockSystem2", function()
    local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
    ...
    if PlayerData.job.onduty then
        ...
        TriggerClientEvent('QBCore:Notify', source, "You're clocked-in", 'success')
		MySQL.Async.insert('INSERT INTO mdt_clocking (user_id, firstname, lastname, clock_in_time, job) VALUES (:user_id, :firstname, :lastname, :clock_in_time, :job) ON DUPLICATE KEY UPDATE user_id = :user_id, firstname = :firstname, lastname = :lastname, clock_in_time = :clock_in_time, job = :job', {
			user_id = PlayerData.citizenid .. PlayerData.job.name,
			firstname = firstName,
			lastname = lastName,
			clock_in_time = time,
