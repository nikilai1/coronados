...
AddEventHandler('qb-communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 100 then
			Teleport(PlayerPedId(), Config.ServiceLocation)
...
function EnableViolentActions()
	...
	DisablePlayerFiring(playerPed,false) -- Disables firing all together if they somehow bypass inzone Mouse Disable
